...
After the updating is finished, we write the content of `.cache` back to `AUTH_CACHE`. We also do it in the Python script. To write a secret, we should encrypt its content using the repo's public key first. This public key can also be acquired by Github APIs (see [this](https://docs.github.com/en/free-pro-team@latest/rest/reference/actions#get-a-repository-public-key)):
